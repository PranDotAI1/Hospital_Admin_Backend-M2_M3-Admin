export const MSG = {
    TOKEN_EXPIRED: "Token Expired..,Please login again for new token",
    INVALID_EMAIL_PASSWORD: "Invalid email or password",
    TOKEN_EXPIRED_MSG: "Token Expired, Please login again",
    API_ERROR: "getting error from api ",
    SERVICE_UNAVAILABLE: "Service unavailable",
    DATA_PROCESSING: "Your data in process. please wait for some time...",
    INTERNAL_SERVER_ERROR: "Internal server error",
    PASSWORD_RESET_SENT:
        "If an account exists with this email, a password reset code/link has been sent.",
    PASSWORD_RESET_INVALID: "Invalid or expired password reset token or OTP",
    PASSWORD_RESET_SUCCESS: "Password has been reset successfully",
    PASSWORD_RATE_LIMITED:
        "Too many password reset requests. Please try again later.",
    PASSWORD_RECENTLY_USED: "Cannot use a recently used password",
    UNAUTHORIZED: "Unauthorized access",
    USER_NOT_FOUND: "User not found",
    INVALID_PASSWORD: "Invalid current password",
};