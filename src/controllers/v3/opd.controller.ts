...
export const scanAndShareWebhook = async (req: Request, res: Response) => {
    try {
        ...
        const requestId =  webhookBody?.response?.requestId || webhookBody?.requestId || generateUID();
        ...
        const abhaAddress = patient?.id || profile?.abhaAddress || patient?.abhaAddress;
        ...
        if (existingVisit) {
            console.log("Duplicate scan detected for ABHA:", abhaAddress, "Existing token:", existingVisit.tokenNumber);
            const existingTokenNumber = existingVisit.tokenNumber;
            await callAbdmOnShare(abhaAddress, context, existingTokenNumber, requestId, req, authorization);
            return;
        }
        ...
        const opdVisitData: Partial<IOPDVisit> = {
            ...
            address: addressObj?.line || addressObj?.line1 ? {
        ...
        console.log("OPDVisit created:", opdVisit._id, "Token:", tokenNumber, "Counter:", context);

        await callAbdmOnShare(abhaAddress, context, tokenNumber, requestId, req, authorization);
...
const callAbdmOnShare = async (
    ...
    try {
        ...
        console.log("ABDM on-share response:", onShareResponse.status, onShareResponse.data);
...
export const completeRegistration = async (req: Request, res: Response) => {
    try {
        ...
        if (manualFields.department !== undefined) updateData.department = manualFields.department;
        if (manualFields.doctorName !== undefined) updateData.doctorName = manualFields.doctorName;
        if (manualFields.consultationFee !== undefined) {
            updateData.consultationFee = Number(manualFields.consultationFee);
        }
        if (manualFields.complaint !== undefined) updateData.complaint = manualFields.complaint;
        if (manualFields.isEmergency !== undefined) updateData.isEmergency = Boolean(manualFields.isEmergency);
        ...
        if (manualFields.payment) {
            updateData.payment = {
                ...
                amount: manualFields.payment.amount ? Number(manualFields.payment.amount) : undefined
            };
        }
        ...
        if (manualFields.mobile !== undefined) updateData.mobile = manualFields.mobile;
        if (manualFields.aadhaarNumber !== undefined) updateData.aadhaarNumber = manualFields.aadhaarNumber;
        if (manualFields.dob !== undefined) updateData.dob = manualFields.dob;
        if (manualFields.gender !== undefined) updateData.gender = manualFields.gender;
...
export const queueStatus = async (req: Request, res: Response) => {
    try {
        console.log("Queue Status request received:", JSON.stringify(req.body, null, 2));
        ...
        const requestId = webhookBody?.requestId || webhookBody?.response?.requestId || generateUID();
        ...
        if (!queueDoc) {
            console.error("Queue not found for context:", context, "date:", todayDate);
            await callAbdmRunningTokenStatus(context, "0", 10, requestId, req);
            return;
        }

        const currentServingToken = queueDoc.currentServingToken.toString().padStart(4, '0');
        const avgServiceTime = queueDoc.avgServiceTime;

        await callAbdmRunningTokenStatus(context, currentServingToken, avgServiceTime, requestId, req);
...
const callAbdmRunningTokenStatus = async (
    ...
    try {
        ...
        console.log("ABDM running token status response:", statusResponse.status, statusResponse.data);
    ...
    try {
        ...
        return res.status(STATUS_CODE.SUCCESS).json({
            ...
            data: {
                ...
                currentServingToken: queueDoc.currentServingToken.toString().padStart(4, '0'),
